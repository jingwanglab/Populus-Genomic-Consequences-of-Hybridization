#!/bin/sh
#SBATCH -c 2 --mem 30G
#SBATCH -o /UserHome/user260/pipeline/hybridization/twisst/twisst.populus.sites1000.out
#SBATCH -e /UserHome/user260/pipeline/hybridization/twisst/twisst.populus.sites1000.err
...
weighting_dir=$twisst_dir/weightings_sites1000
...
elif [ "$window" == "50000" ]; then
##10kb
$python $phyml_tool -g $twisst_dir/populus162.$chr.geno.gz --prefix $twisst_dir/populus162.$chr.phyml.w50kb -w 50000 --windType coordinate --model GTR --phyml $phyml -T 16 -M 100 
...
##sites
$python $phyml_tool -g $twisst_dir/populus162.$chr.geno.gz --prefix $twisst_dir/populus162.$chr.phyml.sites1000 -w 1000 --windType sites --model GTR --phyml $phyml -T 16
...
gunzip $twisst_dir/populus162.$chr.phyml.sites1000.trees.gz
sed '1d' $twisst_dir/populus162.$chr.phyml.sites1000.data.tsv > $twisst_dir/populus162.$chr.phyml.sites1000.data
paste $twisst_dir/populus162.$chr.phyml.sites1000.data $twisst_dir/populus162.$chr.phyml.sites1000.trees |cut -f 7- |gzip - > $twisst_dir/populus162.$chr.phyml.sites1000.shrink.trees.gz
gzip $twisst_dir/populus162.$chr.phyml.sites1000.trees
rm $twisst_dir/populus162.$chr.phyml.sites1000.data
#
cp $twisst_dir/populus162.$chr.phyml.sites1000.shrink.trees.gz $weighting_dir
...
$python $twisst_tool -t $twisst_dir/populus162.$chr.phyml.sites1000.shrink.trees.gz -w $weighting_dir/populus162.$chr.phyml.sites1000.twisst.weights.csv.gz -g Pade -g Pqio -g Pdav -g Prot -g Ptra -g Ptrs -g Palb --method fixed --iterations 100000 --groupsFile $group -D $weighting_dir/populus162.$chr.phyml.sites1000.twisst.distance.csv.gz

elif [ "$step" == "6" ]; then
##summarize the twisst results

zcat $weighting_dir/populus162.chr1.phyml.sites1000.twisst.weights.csv.gz |grep -v "#" > $weighting_dir/chr1.twisst.weights.csv

paste $twisst_dir/populus162.chr1.phyml.sites1000.data.tsv $weighting_dir/chr1.twisst.weights.csv > $weighting_dir/populus162.chr_all.phyml.sites1000.twisst.weights.csv && rm $weighting_dir/chr1.twisst.weights.csv


for chr in chr{2..19}
do
awk '$5>50' $twisst_dir/populus162.$chr.phyml.sites1000.data.tsv |sed '1d' > $weighting_dir/$chr.data.tsv
zcat $weighting_dir/populus162.$chr.phyml.sites1000.twisst.weights.csv.gz |grep -v "#" |sed '1d' > $weighting_dir/$chr.twisst.weights.csv

paste $weighting_dir/$chr.data.tsv $weighting_dir/$chr.twisst.weights.csv >> $weighting_dir/populus162.chr_all.phyml.sites1000.twisst.weights.csv && rm $weighting_dir/$chr.data.tsv $weighting_dir/$chr.twisst.weights.csv
