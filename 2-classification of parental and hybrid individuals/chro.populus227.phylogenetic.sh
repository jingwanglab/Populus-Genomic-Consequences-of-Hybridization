#!/bin/sh
#SBATCH -c 1 --mem 8G
#SBATCH -o /UserHome/user260/pipeline/phylo_tree/mito_chro/populus227/chro.populus227.phylo.out
#SBATCH -e /UserHome/user260/pipeline/phylo_tree/mito_chro/populus227/chro.populus227.phylo.err
...
InputDir="/w/user261/data/sangyupeng/Populus_Unified/chloroplast/c_vcf"
c_vcf="$InputDir/c_merge.vcf.gz"
...
OutDir="/w/user260/multiple_aspen/populus227_vcf/mito_chro/chro"
if [ ! -d "$OutDir" ]; then
mkdir -p $OutDir
fi
...
if [ $step == "1" ]; then 
#remove indels
$vcftools --gzvcf $c_vcf --remove-indels --out $OutDir/$Out_snp_Suffix --recode --recode-INFO-all
...
##with outgroup: 2021 SNPs
##only extract those populus 227 samples without outgroup samples
#without outgroup: 1448 SNPs
...
echo -e "!Title: populus227.outgroup.snp.chro.snp.mdist;" >> $OutDir_phylo/populus227.outgroup.snp.mdist.meg
...
echo -e "!Title: populus227.outgroup.snp.chlo.snp.no_miss.mdist;" >> $OutDir_phylo/populus227.outgroup.snp.no_miss.mdist.meg
